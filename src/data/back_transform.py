...
# In[Load raw data and parse dates]
...
for SPLIT_PARAM in SPLIT_PARAMS:
    ...
    # In[Load the last‐step (deseasoned+diffed+scaled) series & diff orders]
    ...
    # In[Back-transform loop]
    for var, d in diff_order.items():
        print(f"\n=== {var.upper()} ===")

        # 1) load PCA/VAR forecast (scaled+diffed+deseasoned)
